// before Content Script - Alpha Arcade
// Extracts market title and injects inline B4E widget on Alpha Arcade (Algorand)
...
function getMarketTitle() {
  ...
  if (ogTitle && ogTitle.content) {
    return ogTitle.content.replace(/\s*[-|]\s*Alpha Arcade\s*$/i, '').trim();
  }

  const pageTitle = document.title.replace(/\s*[-|]\s*Alpha Arcade\s*$/i, '').trim();
...
function waitAndInject(attempts = 0) {
  ...
  if (title && anchor && window.__b4e) {
    window.__b4e.injectB4EWidget(anchor, title, 'alphaarcade');
    return;
  }
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'getMarketInfo') {
    const title = getMarketTitle();
    if (title) {
      sendResponse({ platform: 'alphaarcade', title, url: getMarketUrl() });
    } else {
      ...
      waitForTitle((t) => sendResponse({ platform: 'alphaarcade', title: t, url: getMarketUrl() }));
    }
    return true;
  }
});
