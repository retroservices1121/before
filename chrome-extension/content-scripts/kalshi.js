...
function getMarketTitle() {
  ...
  // Kalshi URLs often have event tickers like /markets/kxbtc-26apr11-t119999.99
  // or event pages like /events/kxbtc
...
function waitAndInject(attempts = 0) {
  ...
  if (title && anchor && window.__b4e) {
    window.__b4e.injectB4EWidget(anchor, title, 'kalshi');
    return;
  }
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'getMarketInfo') {
    const title = getMarketTitle();
    if (title) {
      sendResponse({ platform: 'kalshi', title, url: getMarketUrl() });
    } else {
      ...
      waitForTitle((t) => sendResponse({ platform: 'kalshi', title: t, url: getMarketUrl() }));
    }
    return true;
  }
});
