// before Content Script - Matchr
// Extracts market title and injects inline B4E widget on Matchr aggregator
...
function getMarketTitle() {
  ...
  if (ogTitle && ogTitle.content) {
    return ogTitle.content.replace(/\s*[-|]\s*Matchr\s*$/i, '').trim();
  }

  const pageTitle = document.title.replace(/\s*[-|]\s*Matchr\s*$/i, '').trim();
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'getMarketInfo') {
    const title = getMarketTitle();
    if (title) {
      sendResponse({ platform: 'matchr', title, url: getMarketUrl() });
    } else {
      ...
      waitForTitle((t) => sendResponse({ platform: 'matchr', title: t, url: getMarketUrl() }));
    }
    return true;
  }
});
